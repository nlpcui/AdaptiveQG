import sys, torch, os, json, configparser, argparse
...
duolingo_en_es_dir = '/Users/cuipeng/Documents/Datasets/duolingo_2018_shared_task/data_en_es' #"/cluster/work/sachan/pencui/duolingo_2018_shared_task/data_en_es"
file_paths = {
...
def cal_metrics(logits, y_labels):
    ...
    y_labels_selected = torch.masked_select(y_labels, valid_positions).detach().numpy()
    y_pos_probs_selected = torch.masked_select(y_pos_probs, valid_positions).detach().numpy()
    y_pred_labels_selected = torch.masked_select(y_pred, valid_positions).detach().numpy()
...
def train(args):
    ...
    # exit(1)
    dataset = DuolingoDataset(os.path.join(duolingo_en_es_dir, file_paths['Duolingo_en_es_train_processed']), shuffle=True)
    ...
    for epoch in range(args.epoch):
        for batch_id, (x_exercise, x_interaction, x_exercise_attn_mask, x_interaction_attn_mask, y_labels) in enumerate(dataloader):
            ...
            logits = model(
                x_exercise=x_exercise,
                x_interaction=x_interaction,
                x_exercise_attn_mask=x_exercise_attn_mask,
                x_interaction_attn_mask=x_interaction_attn_mask,
                y_labels=y_labels
            )
            ...
            loss = loss_function(logits, y_labels)
            loss.backward()
            optimizer.step()
            lr_scheduler.step()
            
            # metrics
            auc, precision, recall, f1_score, accuracy = cal_metrics(logits, y_labels)

            print('In [{}/{}] epoch, [{}/{}] batch, loss:{}, auc:{}, precision:{}, recall:{}, f1_score:{}, accuracy:{}'.format(epoch, args.epoch, batch_id, len(dataset)//args.batch_size, loss, auc, precision, recall, f1_score, accuracy))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', type=str, default=duolingo_en_es_dir)
    parser.add_argument('--data_file', type=str, default=file_paths['Duolingo_en_es_train'])
    parser.add_argument('--vocab_save_path', type=str, default=os.path.join(duolingo_en_es_dir, file_paths['vocab']))
